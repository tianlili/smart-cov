void function (window, factory) {
    ...
    var promise = function () {
        ...
        promise.prototype = {
            ...
            fire: function () {
                var callbacks, values, list = this.list,
                    allResolved = true,
            ...
            if (content) {
                ...
                comboCode.onReady(util.bind(function () {
                    if (comboCode.errorMessage) {
                        ...
                        // );
                        // this.rowsCount = util.splitToLines( beautifyCode
                        // ).length;
                    }
        ...
        return {
            ...
            ControlFrame: function () {
                var document = window.document,
                    controlWindow, hasCreateEmbeded = false,
                    currentMode = "embed",
                    pageBuilder, controllerBuilder;
...
//                        this.fire("show");
                    ...
                    report: function() {
                        ...
                        xmlHttp.onreadystatechange = function() {
                            if (xmlHttp.readyState == 4) {
                            	document.location="mailto:tianlili@baidu.com;?body=http://127.0.0.1/smart-cov/report/data/" + time;
                            }
                        }
                    ...
                    getWindow: function (name) {
                        // name: tracker_main | tracker_page |
                        // tracker_controller
                    ...
                    createEmbed: function () {
                        ...
                        promise.when(
                            hasCreateEmbeded ? [controllerBuilder("embed")] : [pageBuilder(), controllerBuilder("embed")]).then(util.bind(function (pageHtml, controllerHtml) {
                            if (!controllerHtml)
                ...
                return Event.bind({
                    ...
                    showCodeInfo: function (id) {
                        ...
                        elementCodeInfo.innerHTML = util.format(View.templates.controllerCodeInfo, {
                            // id: code.id,
                            // index: ++ codeIndex,
                            fileName: code.fullUrl ? util.format(View.templates.url, {
                                url: code.fullUrl
                            }) : "&#26469;&#33258;&#39029;&#38754;",
                            type: type(code),
                            coverStatement: coverStatement(code.covResult),
    ...
    var restorePageEnvironments = function () {
        ...
        for (i = lastTimerId, down = max(0, lastTimerId - 200); i >= down; i--)
            clearTimeout(i),
        clearInterval(i);
    ...
    // var arrivedSnippetCache, snippetToCodeCache, beginOfLineSnippetCache, code,
    // i, id, l;
    //
    ...
    void function () {
        ...
        View.ControlFrame.pageBuilder(function (html) {
            ...
            util.request(location.href, charset).then(function (html) {
                ...
                AsynStringReplacer.replace(html, allScriptTagRegx, function (raw, openTag, content, closeTag) {
                    var pm, code;

                    pm = new promise;

                    if (srcPropertyRegx.test(openTag)) { // is outer
                        // script
                    ...
                    if (typePropertyRegx.test(openTag) &&
                        RegExp.$1.toLowerCase() != "text/javascript") { // is
                        // not
                        // javascript
                    ...
                    AsynStringReplacer.replace(html, scriptRegx, function (raw, openTag, url, closeTag) {
                        var pm, content;
